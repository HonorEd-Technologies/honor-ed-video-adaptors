export default (elementId: string, config: PlayerOptions) => {
  ...
  return { 
    loadVideoById: (videoId: string, startSeconds?: number, endSeconds?: number) => player.loadVideoById({ videoId, startSeconds, endSeconds }),
    loadVideoByUrl: (mediaContentUrl: string, startSeconds?: number, endSeconds?: number) => player.loadVideoByUrl({ mediaContentUrl, startSeconds, endSeconds }),
    playVideo: () => player.playVideo(),
    pauseVideo: () => player.pauseVideo(),
    stopVideo: () => player.stopVideo(),
    seekTo: (seconds: number, allowSeekAhead: boolean) => player.seekTo(seconds, allowSeekAhead),
    getDuration: () => player.getDuration(),
    getVideoLoadedFraction: () => player.getVideoLoadedFraction(),
    setVolume: (volume: number) =>  player.setVolume(volume),
    getVolume: () => player.getVolume(),
    getPlaybackRate: () => player.getPlaybackRate(),
    setPlaybackRate: (suggestedRate: number) => player.setPlaybackRate(suggestedRate),
...
export type Player = {
  ...
  cueVideoByUrl(options: { mediaContentUrl: string; startSeconds?: number; endSeconds?: number }): void;
