...
import { YoutubeError, YoutubeEvent, YoutubePlayerState } from "../../types/YouTube/YouTubeEvents"

const parseYTPlayerState = (state: YoutubePlayerState): HonorVideoPlayerState | undefined => { 
  switch (state) { 
...
const youtubeStateChangeHandler = (player: HonorPlayer): (event: YoutubeEvent) => void => { 
  ...
  const onStateChange = ({ data }: YoutubeEvent) => { 
    ...
    if (timePoll !== undefined && (YoutubePlayerState.ended === data || YoutubePlayerState.unstarted === data)) { 
...
const youtubeErrorHandler = (player: HonorPlayer): (event: YoutubeEvent) => void => { 
...
export const youtubeEventHandler = (player: HonorPlayer): Object => { 
  const onReady = () => { player.emitter.triggerEvent(HonorVideoEvent.playerReady) }
